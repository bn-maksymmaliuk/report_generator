...
class CsvOutput(BaseOutput):
    ...
    async def write(self, data: list[dict]) -> str:
        ...
        file_path = self.output_dir / f"{self.file_prefix}-{timestamp}.csv"

        if not data:
            logger.warning(f"No rows to write, creating empty file {file_path}")
            payload = ""
        else:
        ...
        async with aiofiles.open(file_path, mode="w", encoding="utf-8", newline="") as file:
